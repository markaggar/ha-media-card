...
**See `LOCAL_SETUP.md` in repository root for actual deployment commands and server paths**

### Media Card Deployment

#### Deployment Process
1. Make changes to modular source files under `src/`
2. Build with `npm run build:concat`
3. Deploy to **test server** first
4. **Hard refresh browser** (Ctrl+Shift+R or Ctrl+F5) to clear cache
5. Test changes thoroughly
6. When stable, deploy to **production server**
7. Check Home Assistant logs for any errors

#### Generic Deployment Command
```powershell
Copy-Item "ha-media-card.js" "\\YOUR_HA_IP\config\www\cards\ha-media-card.js" -Force
```

#### File Locations
- **Development**: `<your_workspace>\ha-media-card.js`
- **HA Server**: `\\YOUR_HA_IP\config\www\cards\ha-media-card.js`
...
**See `LOCAL_SETUP.md` in repository root for actual environment variables and server paths**
