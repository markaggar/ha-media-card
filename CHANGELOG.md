...
- **Burst Panel Uses Media Index Grouping Settings**: When opening the burst panel, the card calls `media_index.get_related_files` without any time/location parameters. If the current photo has already been indexed into a burst group (via `index_burst_groups`), a fast `burst_id` lookup is used. For photos not yet indexed, the service falls back to at-query-time proximity detection using the values configured in the media_index integration options (or safe hardcoded defaults for integrations older than v1.6.0). Per-folder burst group customization via `index_burst_groups` is fully honored — each folder can have different grouping thresholds and the card will always see the correct groups.
...
- **Reolink (and other integration) videos incorrectly removed from queue on HTTP 400**: The browser fires `MEDIA_ERR_SRC_NOT_SUPPORTED` for any HTTP error on a `<source>` element (400, 403, 404, 500 — not just a real 404). Reolink's NVR can return HTTP 400 when it rejects concurrent download requests, e.g. when two cards pre-load the same camera stream at the same time. The card was treating these as "file not found" and permanently removing the items from the navigation queue. Fix: Video items in folder mode now use a transient-failure threshold — the item is skipped silently on the first failure and only permanently removed if it fails again on the second attempt. Genuine missing files (true 404) still get removed after two failures; transient errors recover naturally on the next queue cycle when a fresh URL is resolved.
...
- **Folder Browser Dialog Hidden Behind Card Editor**: Fixed browse button dialog appearing behind the card editor modal
  - Dialog was trapped in HA's card editor stacking context, making folder selection impossible
  - Now uses native HTML `<dialog>` element with `showModal()` for top-layer rendering
  - Browser's top-layer is guaranteed to appear above all other content including HA dialogs
